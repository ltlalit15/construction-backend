...
const createIncident = async (req, res) => {
  ...
  try {
    ...
    // Find the User by userId to get firstName, lastName
...
const getAllIncidents = async (req, res) => {
  try {
    // Populate userId field to include firstName, lastName, and _id of the user
    const incidents = await Incident.find().populate('userId', '_id firstName lastName').sort({ dateTime: -1 });  // Sort by date (newest first)
...
const getIncidentById = async (req, res) => {
  ...
  try {
    // Populate userId field to include firstName, lastName, and _id of the user
    const incident = await Incident.findById(id).populate('userId', '_id firstName lastName');
